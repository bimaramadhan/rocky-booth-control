...
import {getActiveBooth,getEmployeeTodayState,todayWib} from "@/lib/operations";
export default async function Stock(){
  const profile=await requireRole("employee");const admin=createAdminClient();const today=todayWib();const [booth,{stock}]=await Promise.all([getActiveBooth(),getEmployeeTodayState(profile.id)]);
  ...
  if(stock)return <div className="success-panel compact"><div className="success-mark">✓</div><h1>Cek stok sudah selesai</h1><p>Laporan hari ini sudah tersimpan dan tidak dapat dikirim ulang tanpa tindakan admin.</p><a className="btn btn-primary" href="/employee">Kembali ke Beranda</a></div>;
