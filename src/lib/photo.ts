...
export async function processPhoto(file:File,lines:string[]){const {default:sharp}=await import("sharp");validateImage(file,Number(process.env.MAX_PHOTO_BYTES||8_388_608));const input=Buffer.from(await file.arrayBuffer());const clean=await sharp(input,{failOn:"warning"}).rotate().resize({width:1600,height:1600,fit:"inside",withoutEnlargement:true}).webp({quality:78}).toBuffer();const meta=await sharp(clean).metadata();const width=meta.width||1200,height=meta.height||900;const safe=lines.map(x=>x.replace(/[<>&'"]/g,c=>({"<":"&lt;",">":"&gt;","&":"&amp;","'":"&apos;",'"':"&quot;"}[c]!)));
